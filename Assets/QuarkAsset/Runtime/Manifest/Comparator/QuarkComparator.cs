...
namespace Quark.Networking
{
    ...
    public class QuarkComparator
    {
        ...
        bool isEncrypted { get { return QuarkDataProxy.QuarkEncrytionData.QuarkAesEncryptionKeyBytes.Length > 0; } }
        ...
        {
            var uriManifestPath = Path.Combine(QuarkDataProxy.URL, QuarkConstant.MANIFEST_NAME);
        ...
        {
            var localManifestPath = Path.Combine(QuarkDataProxy.PersistentPath, QuarkConstant.MANIFEST_NAME);
...
#endif
                {
                    if (request.isDone)
                    {
                        ...
                        try
                        {
                            if (isEncrypted)
                            {
                                var unencryptedManifest = QuarkUtility.AESDecryptStringToString(context, QuarkDataProxy.QuarkEncrytionData.QuarkAesEncryptionKeyBytes);
        ...
        void OnUriManifestSuccess(string remoteManifestContext)
        {
            ...
            long overallSize = 0;
            var aesKey = QuarkDataProxy.QuarkEncrytionData.QuarkAesEncryptionKeyBytes;
            try
            {
                //解析加密的manifest
                if (isEncrypted)
